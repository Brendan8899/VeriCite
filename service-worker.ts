...
import type { FinalCheckResult, FormatValidationResult, VerifySourceResult, RuntimeMessageType, RuntimeResponse } from './src/types.js';
...
function isRuntimeMessage(message: unknown): message is RuntimeMessageType {
	...
	return (
		msg.type === 'GOOGLE_LOGIN' ||
		msg.type === 'BEGIN_PROCESSING'
	);
}

browser.runtime.onMessage.addListener(async (message: unknown, _sender: any): Promise<RuntimeResponse | undefined >=> {
	if (!isRuntimeMessage(message)) {
		...
		try {
			...
			const scopes = ["https://www.googleapis.com/auth/documents", "https://www.googleapis.com/auth/books"];
