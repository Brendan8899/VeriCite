...
describe('isValidIEEE - format validation', () => {
	...
	test('should validate a correct E-book citation', async () => {
		const normalizedEbookCitation =
			normalizeWhitespace(`[1] L. Bass, P. Clements, and R. Kazman, Software Architecture in Practice, 2nd ed.
	...
	test('should validate a correct Article in Online Encyclopedia citation', async () => {
		const normalizedArticleOnline =
			normalizeWhitespace(`[2] D. Ince, “Acoustic coupler,” in A Dictionary of the Internet. Oxford University
	...
	test('should validate a correct Journal Article Abstract citation', async () => {
		const normalizedJournalArticleAbstract =
			normalizeWhitespace(`[1] M. T. Kimour and D. Meslati, “Deriving objects from use cases in real-time
	...
	test('should validate a correct Journal Article in Scholarly Journal citation', async () => {
		const normalizedScholarlyJournal =
			normalizeWhitespace(`[2] A. Altun, “Understanding hypertext in the context of reading on the web:
	...
	test('should validate a correct Newspaper Article from the Internet citation', async () => {
		const normalizedNewspaperArticle =
			normalizeWhitespace(`[3] C. Wilson-Clark, “Computers ranked as key literacy,” The Atlanta Journal
	...
	test('should validate a correct Professional Internet Site citation', async () => {
		const normalizedProfessionalInternetSite =
			normalizeWhitespace(`[1] European Telecommunications Standards Institute, “Digital Video Broadcasting
	...
	test('should validate a correct General Internet Site', async () => {
		const normalizedGeneralInternetSite =
			normalizeWhitespace(`[2] J. Geralds, “Sega Ends Production of Dreamcast,” vnunet.com, para. 2, Jan. 31,
	...
	test('should validate a correct Lecture citation', async () => {
		const normalizedLectureCitation =
			normalizeWhitespace(`[1] S. Bhanndahar. ECE 4321. Class Lecture, Topic: “Bluetooth can’t help you.”
	...
	test('should validate Single Author Book citation', async () => {
		const normalizedSingleAuthorBook =
			normalizeWhitespace(`[1] W. K. Chen, Linear Networks and Systems. Belmont, CA: WadsworthPress,
        2003.`);
	...
	test('should validate Three or More Authors Book citation', async () => {
		const normalizeMultipleAuthorBook =
			normalizeWhitespace(`[4] R. Hayes, G. Pisano, and S. Wheelwright, Operations, Strategy, and Technical
	...
	test('should validate Organizational Author Book citation', async () => {
		const normalizeOrganizationAuthorBook =
			normalizeWhitespace(`[5] Council of Biology Editors, Scientific Style and Format: The CBE Manual for
	...
	test('should validate Technical Report citation', async () => {
		const normalizeTechnicalReport =
			normalizeWhitespace(`[8] K. E. Elliott and C. M. Greene, “A local adaptive protocol,” Argonne National
	...
	test('should validate Conference Paper citation', async () => {
		const normalizeConferencePaper =
			normalizeWhitespace(`[12] J. Smith, R. Jones, and K. Trello, “Adaptive filtering in data communications with
