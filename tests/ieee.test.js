...
describe('isValidIEEE - format validation', () => {
	...
	// The newline character is removed as reconstituteIEEEReferences function have handled it
	// Replaced all URL with http://www.google.com to prevent test flakiness

	test('should validate a correct E-book citation', async () => {
		const normalizedEbookCitation = normalizeWhitespace(
		...
		const result = await isValidIEEE(normalizedEbookCitation);
		expect(result.valid).toBe(true);
		expect(result.errors).toHaveLength(0);
	});

	test('should validate a correct Article in Online Encyclopedia citation', async () => {
		const normalizedArticleOnline = normalizeWhitespace(
			`[2] D. Ince, “Acoustic coupler,” in A Dictionary of the Internet. Oxford University Press, [online document], 2001. Available: Oxford Reference Online, http://www.google.com [Accessed: May 24, 2007].`,
		);
	...
	test('should validate a correct Journal Article Abstract citation', async () => {
		const normalizedJournalArticleAbstract =
			normalizeWhitespace(`[1] M. T. Kimour and D. Meslati, “Deriving objects from use cases in real-time embedded systems,” Information and Software Technology, vol. 47, no. 8, p. 533, June 2005. [Abstract]. Available: ProQuest, http://www.google.com.
	...
	test('should validate a correct Journal Article in Scholarly Journal citation', async () => {
		const normalizedScholarlyJournal = normalizeWhitespace(
			`[2] A. Altun, “Understanding hypertext in the context of reading on the web: Language learners’ experience,” Current Issues in Education, vol. 6, no. 12, July, 2005. [Online serial]. Available: http://www.google.com. [Accessed Dec. 2, 2007].`,
		);
	...
	test('should validate a correct Newspaper Article from the Internet citation', async () => {
		const normalizedNewspaperArticle = normalizeWhitespace(
			`[3] C. Wilson-Clark, “Computers ranked as key literacy,” The Atlanta Journal Constitution, para. 3, March 29, 2007. [Online], Available: http://www.google.com. [Accessed Sept. 18, 2007].`,
		);
	...
	test('should validate a correct Professional Internet Site citation', async () => {
		const normalizedProfessionalInternetSite = normalizeWhitespace(
			`[1] European Telecommunications Standards Institute, “Digital Video Broadcasting (DVB): Implementation guide for DVB terrestrial services; transmission aspects,” European Telecommunications Standards Institute, ETSI-TR-101, 2007. [Online]. Available: http://www.google.com. [Accessed: Nov. 12, 2007].`,
		);
	...
	test('should validate a correct General Internet Site', async () => {
		const normalizedGeneralInternetSite = normalizeWhitespace(
			`[2] J. Geralds, “Sega Ends Production of Dreamcast,” vnunet.com, para. 2, Jan. 31, 2007. [Online]. Available: http://www.google.com. [Accessed Sept. 12, 2007].`,
		);
	...
	test('should validate a correct Email citation', async () => {
		const normalizedEmailCitation = normalizeWhitespace(
	...
	test('should validate a correct Lecture citation', async () => {
		const normalizedLectureCitation = normalizeWhitespace(
	...
	test('should validate Single Author Book citation', async () => {
		const normalizedSingleAuthorBook = normalizeWhitespace(
	...
	test('should validate Three or More Authors Book citation', async () => {
		const normalizeMultipleAuthorBook = normalizeWhitespace(
	...
	test('should validate Organizational Author Book citation', async () => {
		const normalizeOrganizationAuthorBook = normalizeWhitespace(
	...
	test('should validate Technical Report citation', async () => {
		const normalizeTechnicalReport = normalizeWhitespace(
	...
	test('should validate Conference Paper citation', async () => {
		const normalizeConferencePaper = normalizeWhitespace(
			`[12] J. Smith, R. Jones, and K. Trello, “Adaptive filtering in data communications with self improved error reference,” In Proc. IEEE International Conference on Wireless Communications ’04, 2004, pp. 65`,
		);
