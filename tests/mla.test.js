...
describe('MLA journal article format', () => {
  ...
  test('fails when year appears right after author (APA style mistake)', async () => {
    const citation = 'Smith, John. (2021). "Article Title." Journal Name, vol. 12, no. 3, pp. 45–67.';
    const result = await isValidMLA(citation);
    expect(result.valid).toBe(false);
    expect(result.errors).toContain('Year must not appear in parentheses after the author — this is APA format');
  });
...
describe('MLA website format', () => {
  ...
  test('passes a well-formed website citation', async () => {
    const citation = 'Smith, John. "Page Title." Website Name, 15 Jan. 2021, https://www.example.com/page.';
    const result = await isValidMLA(citation);
    expect(result.valid).toBe(true);
  });
  ...
  test('passes citation with Accessed date notation', async () => {
    const citation = 'Smith, John. "Page Title." Website Name, 15 Jan. 2021, https://example.com. Accessed 20 May 2026.';
    const result = await isValidMLA(citation);
    expect(result.valid).toBe(true);
  });
...
describe('MLA book format', () => {
  ...
  test('passes a book chapter with editor', async () => {
    const citation = 'Smith, John. "Chapter Title." Book Title, edited by Jane Doe, Publisher, 2021, pp. 45–67.';
    const result = await isValidMLA(citation);
    expect(result.valid).toBe(true);
  });
